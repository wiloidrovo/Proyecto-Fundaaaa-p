...
# Everything in pygame runs in a while loop.
    while run:
        ...
        for obstacle in obstacles: # We call the draw and update function on every
            # print("distace X: dog - ", player.X_POSITION, "obstacle - ", obstacle.rect.x)
            # print("distace Y: dog - ", player.Y_POSITION, "obstacle - ", obstacle.rect.y)
            # print("distance dog: ", player.X_POSITION, player.Y_POSITION)
            print("distace obstacle: ", obstacle.rect.x, obstacle.rect.y)
            while obstacle.rect.x <= player.X_POSITION * 2:                
                #350 = normal, 287 = bate
                if obstacle.rect.y >= player.Y_POSITION:
                    print("SALTA")
                    break
                else:
                    print("AGACHA")
